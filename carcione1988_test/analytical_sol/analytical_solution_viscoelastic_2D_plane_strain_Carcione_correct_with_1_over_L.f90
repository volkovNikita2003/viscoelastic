!! modified by Nikita Volkov to reproduce the problem statement from the Carcione 1988
...
! given in Carcione et al. 1988 GJI vol 95 p 604 Table 1
  double precision, parameter :: Vp = 3190.226435988779d0
  double precision, parameter :: Vs = 2175.5894496415513d0
...
! definition position recepteur Carcione
  ...
  double precision :: x1_read, x2_read
...
! output files
  ...
  character(len=32)  :: sx1, sx2

! ********** end of variable declarations ************

! classical least-squares constants
  tau_epsilon_nu1 =  (/ 0.0325305d0, 0.0032530d0 /)
  tau_sigma_nu1 = (/ 0.0311465d0, 0.0031146d0 /)

  tau_epsilon_nu2 = (/ 0.0332577d0, 0.0033257d0 /)
  tau_sigma_nu2 = (/ 0.0304655d0, 0.0030465d0 /)
...
! reading point
  ...
  print *, 'x1_read = ', x1_read
  print *, 'x2_read = ', x2_read
...
!   M2C = (M2_unrelaxed /(sum(tau_epsilon_nu2(:)/tau_sigma_nu2(:)))) * temp
  ...
  temp = dcmplx(0.d0,0.d0)
  do i = 1, Lnu
  temp = temp + dcmplx(1.d0, omega*tau_epsilon_nu1(i)) / &
                dcmplx(1.d0, omega*tau_sigma_nu1(i))
  enddo
  M1C = M1_relaxed * (1.d0 - dble(Lnu) + temp)

  temp = dcmplx(0.d0,0.d0)
  do i = 1, Lnu
  temp = temp + dcmplx(1.d0, omega*tau_epsilon_nu2(i)) / &
                dcmplx(1.d0, omega*tau_sigma_nu2(i))
  enddo
  M2C = M2_relaxed * (1.d0 - dble(Lnu) + temp)
...
! save time result inverse FFT for Vx
  ...
  if (TURN_ATTENUATION_OFF) then
    if (DO_NOT_COMPUTE_THE_NEAR_FIELD) then
      filename = 'Vx_time_analytical_solution_elastic_without_near_field_' // &
           trim(adjustl(sx1)) // '_' // &
           trim(adjustl(sx2)) // '.dat'
    else
      filename = 'Vx_time_analytical_solution_elastic_' // &
           trim(adjustl(sx1)) // '_' // &
           trim(adjustl(sx2)) // '.dat'
    endif
  else
    if (DO_NOT_COMPUTE_THE_NEAR_FIELD) then
      filename = 'Vx_time_analytical_solution_viscoelastic_without_near_field_' // &
           trim(adjustl(sx1)) // '_' // &
           trim(adjustl(sx2)) // '.dat'
    else
      filename = 'Vx_time_analytical_solution_viscoelastic_' // &
           trim(adjustl(sx1)) // '_' // &
           trim(adjustl(sx2)) // '.dat'
    endif
  endif
...
! thus keeping the useful part of the signal only (the first six seconds of the seismogram)
        if (time >= 0.d0 .and. time <= 0.5d0) write(11,*) sngl(time),real(c(it))
  enddo
  close(11)
...
! save time result inverse FFT for Vz
  ...
  if (TURN_ATTENUATION_OFF) then
    if (DO_NOT_COMPUTE_THE_NEAR_FIELD) then
      filename = 'Vz_time_analytical_solution_elastic_without_near_field_' // &
           trim(adjustl(sx1)) // '_' // &
           trim(adjustl(sx2)) // '.dat'
    else
      filename = 'Vz_time_analytical_solution_elastic_' // &
           trim(adjustl(sx1)) // '_' // &
           trim(adjustl(sx2)) // '.dat'
    endif
  else
    if (DO_NOT_COMPUTE_THE_NEAR_FIELD) then
      filename = 'Vz_time_analytical_solution_viscoelastic_without_near_field_' // &
           trim(adjustl(sx1)) // '_' // &
           trim(adjustl(sx2)) // '.dat'
    else
      filename = 'Vz_time_analytical_solution_viscoelastic_' // &
           trim(adjustl(sx1)) // '_' // &
           trim(adjustl(sx2)) // '.dat'
    endif
  endif
